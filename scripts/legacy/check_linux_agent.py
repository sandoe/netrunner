...
async def main():
    ...
    for nid, node in nodes.items():
        if node.get("name") in ["Linux"]:
            ...
            print(f"Checking agent on {nid}...")
            
            pwd = creds.get("password", "")
            escaped_pwd = pwd.replace("'", "'\\''")
            cmd = f"echo '{escaped_pwd}' | sudo -S systemctl status netrunner-agent; echo '{escaped_pwd}' | sudo -S journalctl -u netrunner-agent -n 20 --no-pager"
