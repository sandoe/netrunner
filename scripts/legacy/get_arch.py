...
async def main():
    ...
    for nid, node in nodes.items():
        if node.get("name") == "rpi":
            ...
            res, err = await session_manager.run(nid, node_with_creds, ["uname -m", "file /usr/local/bin/netrunner-agent", "ls -l /usr/local/bin/netrunner-agent"])
            print(f"RPI arch info: {res}")
