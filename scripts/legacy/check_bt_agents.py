...
async def main():
    ...
    for nid, node in nodes.items():
        if node.get("name") in ["rpi", "Linux"]:
            creds = await _get_node_with_creds(nid, nodes)
            print(f"Checking agent on {nid} ({node.get('name')})...")
            try:
                res = await session_manager.run(nid, creds, ["systemctl status netrunner-agent", "journalctl -u netrunner-agent -n 10 --no-pager", "sudo btmgmt find"])
                print(f"Result: {res}")
