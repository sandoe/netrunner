...
async def get_netbox_devices(config: NetboxConfig):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise HTTPException(status_code=e.response.status_code, detail=f"NetBox API error: {e}")
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Failed to connect to NetBox: {e}")
