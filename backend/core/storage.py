...
async def _rotate_influxdb():
    ...
    try:
        # 1. Cold Storage Archival Phase (Zero-Trust Data Lake)
        archive_dir = DATA_DIR / "cold_storage" / datetime.utcnow().strftime("%Y/%m/%d")
        archive_dir.mkdir(parents=True, exist_ok=True)

        # Simulate exporting to Parquet/MinIO
        archive_file = archive_dir / f"telemetry_{int(time.time())}.parquet.snappy"

        # In a real implementation we would run:
        # query = f'from(bucket:"{INFLUXDB_BUCKET}") |> range(start: {start}, stop: {end})'
        # result = await query_api.query_data_frame(query, org=INFLUXDB_ORG)
        # result.to_parquet(archive_file, engine="pyarrow", compression="snappy")

        # Write a dummy file to represent the archival process completing
        archive_file.write_text(
            "MAGIC_PARQUET_DATA: Simulated XDR Data Lake Archival Tier Export"
        )
        logging.info(
            f"[Cold Storage] Successfully archived hot telemetry to {archive_file}"
        )

        # 2. Hot Tier Deletion Phase
        await delete_api.delete(
            start, end, "", bucket=INFLUXDB_BUCKET, org=INFLUXDB_ORG
        )
        record_event(
            "warning",
            "system",
            "STORAGE",
            "Storage Limit",
            f"InfluxDB limit exceeded. Archived to Cold Storage and rotated data older than 24h.",
        )
    ...
    try:
        async for session in get_db():
            ...
            await session.execute(
                text(
                    "DELETE FROM events WHERE id IN (SELECT id FROM events ORDER BY created_at ASC LIMIT 10000)"
                )
            )
        ...
        record_event(
            ...
            "STORAGE",
            "Storage Limit",
            f"Postgres limit exceeded. Auto-rotated oldest events.",
        )
    except Exception as e:
...
async def _rotate_logs():
    ...
    try:
        ...
        record_event(
            ...
            "STORAGE",
            "Storage Limit",
            f"Logs limit exceeded. Auto-rotated log files.",
        )
    except Exception as e:
