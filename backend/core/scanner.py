...
async def run_remote_nmap_xml(nid: str, node: dict, target: str, profile: str = "quick", sudo_pass: str = "") -> dict:
    ...
    if not output:
        raise RuntimeError(f"No output from nmap scan. Raw results: {results}, Cmd: {cmd}")

    xml_start = output.find("<?xml")
    if xml_start == -1:
        if "command not found" in output.lower() or "not found" in output.lower():
            raise RuntimeError("Nmap is not installed on this node. Please install it first.")
        raise RuntimeError(f"Failed to parse nmap output as XML. Output: {output[:200]}")
