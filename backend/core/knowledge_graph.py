...
class KnowledgeGraph:
    ...
    def rebuild_from_state(self, nodes_db: dict, connections: dict, creds: dict):
        self.graph.clear()
        ...
        for nid, node_data in nodes_db.items():
            ...
            self.graph.add_node(
                nid, name=name, ip=ip, device_type=dtype, type="Machine"
            )
        ...
        for nid, cred_data in creds.items():
            if isinstance(cred_data, tuple) and len(cred_data) == 2:
                ...
                self.graph.add_node(
                    cred_id, name=f"{u}:{p}", username=u, password=p, type="Credential"
                )
    ...
    def get_vis_json(self):
        ...
        for u, v, attrs in self.graph.edges(data=True):
            edges.append(
                {"from": str(u), "to": str(v), "label": attrs.get("label", "")}
            )
