...
class CSIGenerator:
    def __init__(self):
        ...
        self.amplitudes = [
            random.uniform(10.0, 30.0) for _ in range(self.num_subcarriers)
        ]
        self.running = False
    ...
    async def start(self):
        ...
        while self.running:
            ...
            if (
                not self.motion_active and random.random() < 0.05
            ):  # 5% chance per tick to start motion
            ...
            if (
                not self.typing_active
                and not self.motion_active
                and random.random() < 0.08
            ):  # 8% chance to type a key
...
class RealCSIGenerator:
    ...
    async def start(self):
        ...
        self.transport, _ = await loop.create_datagram_endpoint(
            lambda: BeaconProtocol(), local_addr=("0.0.0.0", 8001)
        )
