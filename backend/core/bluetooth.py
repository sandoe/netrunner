...
async def bluetooth_scanner_loop():
    ...
    def detection_callback(device, advertisement_data):
        ...
        if not prev:
            ...
            if prev.get("name", "Unknown Device") == "Unknown Device" and name != "Unknown Device":
                prev["name"] = name
                should_emit = True
    ...
    try:
        ...
        while True:
            ...
            now = time.time()
            stale_macs = [mac for mac, data in discovered_devices.items() if now - data["last_seen"] > 60]
...
async def aoa_serial_loop():
    ...
    try:
        ...
        logger.warning("pyserial-asyncio not installed. AoA Direction Finding hardware support disabled.")
        return
