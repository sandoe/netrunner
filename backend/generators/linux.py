...
def gen_package(
    pkgs: list[str], action: str = "install", manager: str = "auto"
) -> list[str]:
    ...
    apt_action = {
        "install": f"install -y {pkg_str}",
        "remove": f"remove -y {pkg_str}",
        "purge": f"purge -y {pkg_str}",
    }[action]
    yum_action = {
        "install": f"install -y {pkg_str}",
        "remove": f"remove -y {pkg_str}",
        "purge": f"remove -y {pkg_str}",
    }[action]
    pacman_action = {
        "install": f"-S --noconfirm {pkg_str}",
        "remove": f"-R --noconfirm {pkg_str}",
        "purge": f"-Rns --noconfirm {pkg_str}",
    }[action]
    apk_action = {
        "install": f"add {pkg_str}",
        "remove": f"del {pkg_str}",
        "purge": f"del {pkg_str}",
    }[action]
...
def gen_cron(cfg: dict) -> list[str]:
    ...
    if action == "list":
        cmds.append(
            f"crontab -l -u {user} 2>/dev/null || echo '(no crontab for {user})'"
        )
        ...
        cmds.append(
            f"crontab -r -u {user} 2>/dev/null || true; echo 'Crontab cleared for {user}'"
        )
...
def gen_file_write(cfg: dict) -> list[str]:
    ...
    if backup:
        cmds.append(
            f"cp {shlex.quote(path)} {shlex.quote(path + '.netrunner_bak')} 2>/dev/null || true"
        )
...
def gen_ssh_hardening(cfg: dict) -> list[str]:
    ...
    if allowed_users:
        ...
        cmds.append(
            f"grep -q '^AllowUsers' /etc/ssh/sshd_config && sed -i 's/^AllowUsers .*/AllowUsers {users}/' /etc/ssh/sshd_config || echo 'AllowUsers {users}' >> /etc/ssh/sshd_config"
        )
...
def gen_remote_desktop(cfg: dict) -> list[str]:
    ...
    if protocol == "remmina":
        cmds = [
            f"# ── Remote Desktop Client: {action.upper()} REMMINA ───────────────────"
        ]
        if action == "uninstall":
            cmds += [
                "# Purge packages",
    ...
    cmds = [
        f"# ── Remote Desktop: {action.upper()} {protocol.upper()} on port {port} ───────────────────"
    ]
    ...
    desktop_start_cmd = (
        "startxfce4"
        if desktop == "xfce"
        else ("startlxde" if desktop == "lxde" else "xterm")
    )
