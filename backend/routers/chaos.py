    ...
    while True:
        ...
        if attack_choice == "scapy":
            payload_script = RedTeamGenerator.generate_dns_spoof_payload(
                target_domain="netrunner.corp", spoofed_ip="10.99.99.99"
            )
        ...
        asyncio.create_task(
            execute_chaos_wrapper(nid, node, payload_script, log_desc, target_ip)
        )
...
async def api_trigger_manual_attack(req: ManualAttackRequest, user: dict = Depends(require_non_student)):
    nodes = await load_nodes_db()
    ...
    attacker_geo = await get_ip_geolocation(
        req.attacker_ip, default_name="Custom Attacker"
    )
...
async def api_apply_chaos_shaping(nid: str, payload: ShapingPayload):
    ...
    if payload.latency_ms > 0:
        netem_parts.append(
            f"delay {payload.latency_ms}ms {payload.jitter_ms}ms distribution normal"
        )
