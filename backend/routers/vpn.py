...
def get_wg_status():
    try:
        res = subprocess.run(["wg", "show", "wg0"], capture_output=True, text=True, check=True)
        return res.stdout
...
def get_server_pubkey():
    try:
        res = subprocess.run(["wg", "show", "wg0", "public-key"], capture_output=True, text=True, check=True)
    ...
    if status_text:
        ...
        for line in status_text.splitlines():
            ...
            if line.startswith("peer:"):
                ...
                current_peer = {"pubkey": line.split()[1], "endpoint": "", "allowed_ips": "", "latest_handshake": ""}
...
async def api_vpn_create_client(body: VpnClientCreate):
    ...
    try:
        subprocess.run(["wg", "set", "wg0", "peer", pub, "allowed-ips", f"{ip}/32"], check=True, capture_output=True)
    except subprocess.CalledProcessError as e:
        raise HTTPException(500, f"Failed to apply wg rule: {e.stderr.decode() if e.stderr else str(e)}")
...
async def api_vpn_delete_client(pubkey: str):
    subprocess.run(["wg", "set", "wg0", "peer", pubkey, "remove"], check=False, capture_output=True)
        ...
        for i, line in enumerate(lines):
            if line.strip() == "[Peer]":
                ...
                for j in range(i+1, min(i+5, len(lines))):
                    if lines[j].strip().startswith("[Peer]") or lines[j].strip().startswith("[Interface]"): break
                    if pubkey in lines[j]:
                        is_target = True
                        break
                if is_target:
                    skip = True
                    if len(new_lines) > 0 and new_lines[-1].strip().startswith("# Client:"):
                        new_lines.pop()
                    continue
