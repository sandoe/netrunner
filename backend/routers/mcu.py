...
async def ensure_mcu_cli(node_id: str, node: dict):
    ...
    await session_manager.upload_file(node_id, node, "/app/backend/scripts/mpremote_api.py", "/tmp/nr_scripts/mpremote_api.py")
    await session_manager.upload_file(node_id, node, "/app/backend/scripts/nr_mcu_cli.py", "/tmp/nr_scripts/nr_mcu_cli.py")

@router.get("/{node_id}/files")
async def list_mcu_files(node_id: str, port: str, path: str = ".", sudo_password: str = "", user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    ...
    async with yield_port(node_id, port):
        ...
        if sudo_password:
            await session_manager.run(node_id, nodes[node_id], [f"echo '{sudo_password}' | sudo -S chmod 666 {port}"])
    ...
    try:
        ...
        try:
            json_line = next(line for line in reversed(lines) if line.startswith('[') or line.startswith('{'))
            parsed = json.loads(json_line)
        except (StopIteration, json.JSONDecodeError):
            raise HTTPException(status_code=500, detail=f"Failed to parse ls output. Raw: {output}")
            
        if isinstance(parsed, dict) and "error" in parsed:
            raise HTTPException(status_code=500, detail=parsed["error"])
            
        files = [{"name": e["name"], "is_dir": e["is_dir"], "size": e["size"]} for e in parsed]
        return {"files": files}
...
async def read_mcu_file(node_id: str, port: str, path: str, sudo_password: str = "", user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
...
async def write_mcu_file(node_id: str, payload: McuWritePayload, user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
    ...
    if payload.sudo_password:
        await session_manager.run(node_id, nodes[node_id], [f"echo '{payload.sudo_password}' | sudo -S chmod 666 {payload.port}"])
    ...
    try:
        ...
        success, upload_err = await session_manager.upload_file(node_id, nodes[node_id], temp_path, '/tmp/nr_upload.tmp')
        if not success:
            raise HTTPException(status_code=500, detail=f"File upload failed: {upload_err}")
            
        cmds = [f"timeout 15 python3 /tmp/nr_scripts/nr_mcu_cli.py {payload.port} write {payload.path} /tmp/nr_upload.tmp"]
...
async def upload_lib(node_id: str, port: str = Form(...), path: str = Form("/"), sudo_password: str = Form(""), file: UploadFile = File(...), user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
    ...
    try:
        ...
        success, upload_err = await session_manager.upload_file(node_id, nodes[node_id], local_zip, remote_zip)
        if not success:
            raise HTTPException(status_code=500, detail=f"ZIP upload failed: {upload_err}")
...
async def run_script(node_id: str, payload: RunScriptPayload, user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
    ...
    try:
        ...
        success, upload_err = await session_manager.upload_file(node_id, nodes[node_id], temp_path, remote_path)
        if not success:
            raise HTTPException(status_code=500, detail=f"File upload failed: {upload_err}")
    finally:
    ...
    if payload.language == "micropython":
        ...
        await ensure_mcu_cli(node_id, nodes[node_id])
        if payload.sudo_password:
            cmds.append(f"echo '{payload.sudo_password}' | sudo -S chmod 666 {payload.port}")
        cmds.append(f"timeout 15 python3 /tmp/nr_scripts/nr_mcu_cli.py {payload.port} run {remote_path}")
    elif payload.language == "arduino":
        cmds.append("mkdir -p /tmp/nr_arduino_sketch")
        cmds.append(f"mv {remote_path} /tmp/nr_arduino_sketch/nr_arduino_sketch.ino")
        cmds.append(f"arduino-cli compile --fqbn arduino:avr:uno /tmp/nr_arduino_sketch")
        cmds.append(f"arduino-cli upload -p {payload.port} --fqbn arduino:avr:uno /tmp/nr_arduino_sketch")
    elif payload.language == "rust":
        ...
        cmds.append(f"cd /tmp/nr_rust_proj && espflash flash --port {payload.port} target/release/nr_rust_proj")
    else:
        raise HTTPException(status_code=400, detail="Unsupported language")
...
async def reset_mcu(node_id: str, payload: PortPayload, user=Depends(get_current_user)):
    ...
    sudo_prefix = f"echo '{payload.sudo_password}' | sudo -S " if payload.sudo_password else ""
    ...
    if results and results[0].get("output", "").strip():
        ...
        for pid in pids:
            ...
            if res and "yes" in res[0].get("output", ""):
                await session_manager.run(node_id, nodes[node_id], [f"{sudo_prefix}kill -USR1 {pid}"])
                return {"status": "ok", "message": "Sent SIGUSR1 to active serial monitor"}
...
async def delete_mcu_file(node_id: str, port: str, path: str, sudo_password: str = "", user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
...
async def mkdir_mcu(node_id: str, payload: McuMkdirPayload, user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
...
async def setup_ota(node_id: str, payload: OtaPayload, user=Depends(get_current_user)):
    ...
    try:
        ...
        success, upload_err = await session_manager.upload_file(node_id, nodes[node_id], temp_path, '/tmp/ota_firmware.py')
        if not success:
            raise HTTPException(status_code=500, detail=f"OTA setup failed: {upload_err}")
    finally:
        if 'temp_path' in locals() and os.path.exists(temp_path):
            os.remove(temp_path)
...
async def get_ota(node_id: str):
    ...
    results, err = await session_manager.run(node_id, nodes[node_id], ["cat /tmp/ota_firmware.py || echo ''"])
    if err:
        raise HTTPException(status_code=500, detail=str(err))
...
async def install_mcu_tools(node_id: str, payload: InstallToolsPayload, user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
    ...
    if python_tools:
        ...
        cmds.append(f"pip3 install --user --break-system-packages {tools_str} || pip install --user --break-system-packages {tools_str} || pip install {tools_str}")
        
    if "espflash" in rust_tools:
        cmds.append("cargo install espflash")
    if "probe-rs" in rust_tools:
        cmds.append("curl --proto '=https' --tlsv1.2 -LsSf https://github.com/probe-rs/probe-rs/releases/latest/download/probe-rs-tools-installer.sh | sh")
...
async def flash_firmware(node_id: str, payload: FlashFirmwarePayload, user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
    ...
    try:
        ...
        success, upload_err = await session_manager.upload_file(node_id, nodes[node_id], temp_path, '/tmp/firmware_upload.bin')
        if not success:
            raise HTTPException(status_code=500, detail=f"Firmware upload failed: {upload_err}")
    finally:
        if os.path.exists(temp_path):
            os.remove(temp_path)
    ...
    full_output = "\n".join([r.get("output", "") + (("\nError: " + r.get("error")) if r.get("error") else "") for r in results])
    for r in results:
        if r.get("error") or "error" in r.get("output", "").lower() or "not found" in r.get("output", "").lower():
...
async def flash_workspace(node_id: str, payload: FlashWorkspacePayload, user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
    ...
    if not os.path.exists(bin_path):
        raise HTTPException(status_code=404, detail=f"Compiled firmware not found at {bin_path}. Did the build succeed?")
        
    remote_bin = "/tmp/nr_workspace_fw.bin"
    success, upload_err = await session_manager.upload_file(node_id, nodes[node_id], bin_path, remote_bin)
    if not success:
        raise HTTPException(status_code=500, detail=f"Firmware upload failed: {upload_err}")
...
async def flash_auto(node_id: str, payload: FlashAutoPayload, user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected")
