...
async def _get_sftp_client(nid: str):
    ...
    if transport != "ssh":
        raise HTTPException(status_code=400, detail="File explorer requires SSH transport")
...
async def list_directory(nid: str, path: str = Query("/", description="Path to list")):
    ...
    try:
        ...
        raise HTTPException(status_code=403, detail=f"Permission denied or path not found: {e}")
    finally:
        sftp.close()
        client.close()
...
async def read_file(nid: str, path: str = Query(..., description="Path to read")):
    ...
    try:
        ...
        if attr.st_size > 5 * 1024 * 1024: # 5 MB limit
            raise HTTPException(status_code=400, detail="File too large to preview (limit 5MB)")
        ...
        raise HTTPException(status_code=403, detail=f"Permission denied or file not found: {e}")
    except Exception as e:
