...
async def _run_attack_bg(nid: str, node: dict, service: str, payload: dict):
    ...
    if not _validate_host(target_ip):
        active_attacks[nid] = {
            "status": "failed",
            "message": f"Invalid target host: {target_ip}",
        }
        return
    ...
    if not _validate_service(service):
        active_attacks[nid] = {
            "status": "failed",
            "message": f"Invalid service name: {service}",
        }
        return
    ...
    try:
        ...
        if not hasattr(session, "run_command"):
            active_attacks[nid] = {
                "status": "failed",
                "message": "Session does not support running commands",
            }
            return
        ...
        if attack_mode == "wordlist":
            ...
            if not wordlist_name or not _validate_filename(wordlist_name):
                active_attacks[nid] = {
                    "status": "failed",
                    "message": f"Invalid wordlist filename: {wordlist_name}",
                }
                return
            ...
            if not wordlist_path.exists():
                active_attacks[nid] = {
                    "status": "failed",
                    "message": f"Wordlist {wordlist_name} not found",
                }
                return
            ...
            if not _validate_filename(wordlist_name):
                ...
                return
            cmd_upload_wl = (
                f"echo {encoded_wordlist} | base64 -d > {_shell_quote(remote_wordlist)}"
            )
            ...
            if not _validate_filename(algo_user):
                active_attacks[nid] = {
                    "status": "failed",
                    "message": f"Invalid algo_user: {algo_user}",
                }
                return
            if not re.match(r"^[a-zA-Z0-9]+$", str(algo_charset)):
                active_attacks[nid] = {
                    "status": "failed",
                    "message": f"Invalid algo_charset: {algo_charset}",
                }
                return
            if (
                not isinstance(algo_min, int)
                or not isinstance(algo_max, int)
                or algo_min < 1
                or algo_max > 10
                or algo_min > algo_max
            ):
                active_attacks[nid] = {
                    "status": "failed",
                    "message": f"Invalid algo_min/algo_max: {algo_min}/{algo_max}",
                }
                return
        ...
        start_event = {
            ...
            "source": {
                "ip": "127.0.0.1",
                "city": "Netrunner Console",
                "lat": 55.67,
                "lng": 12.56,
            },
            "target": {
                "ip": target_ip,
                "city": node.get("name", "Target"),
                "lat": 55.68,
                "lng": 12.57,
            },
        ...
        while active_attacks.get(nid, {}).get("status") in ("running", "paused"):
            ...
            if active_attacks[nid]["status"] == "paused":
                continue
            log_out = session.run_command(
                f"tail -n 20 {_shell_quote(log_file)} 2>/dev/null"
            )
                ...
                if line.startswith("{"):
                    try:
                        ...
                        if "success" in data:
                            ...
                            event_type = f"Brute Force ({service.upper()}) " + (
        ...
        if found_creds:
            await store_credentials(
                nid, found_creds["username"], found_creds["password"]
            )
            active_attacks[nid] = {
                "status": "success",
...
async def launch_attack(payload: dict):
    ...
    active_attacks[nid] = {
        ...
        "wordlist": (
            wordlist
            if payload.get("attack_mode") == "wordlist"
            else f"ALGO ({payload.get('algo_min')}-{payload.get('algo_max')} chars)"
        ),
...
async def stop_attack(payload: dict):
    ...
    if session and pid and pid.isdigit():
        try:
            ...
            session.run_command(
                "docker ps -q --filter ancestor=secsi/hydra | xargs -r docker stop 2>/dev/null"
            )
        except:
            pass
...
async def pause_attack(payload: dict):
    ...
    if session and pid and pid.isdigit():
        try:
            ...
            session.run_command(
                "docker ps -q --filter ancestor=secsi/hydra | xargs -r docker pause 2>/dev/null"
            )
        except:
            pass
...
async def resume_attack(payload: dict):
    ...
    if session and pid and pid.isdigit():
        try:
            ...
            session.run_command(
                "docker ps -q --filter ancestor=secsi/hydra | xargs -r docker unpause 2>/dev/null"
            )
        except:
            pass
