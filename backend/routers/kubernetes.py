...
def generate_mock_k8s_data() -> Dict[str, Any]:
    ...
    for i in range(num_pods):
        ...
        status = random.choices(
            ["Running", "Pending", "CrashLoopBackOff", "Completed"],
    ...
    for ns in namespaces:
        ...
        for i in range(num_deps):
            ...
            available = (
                ...
                else max(0, desired - random.randint(1, desired))
            )
            deployments.append(
                {
    ...
    return {
        ...
        "cluster_health": (
            "Degraded"
            if any(p["status"] == "CrashLoopBackOff" for p in pods)
            else "Healthy"
        ),
        "total_cpu": sum(n["cpu_capacity"] for n in nodes),
        "total_memory_gb": sum(
            int(n["memory_capacity"].replace("Gi", "")) for n in nodes
        ),
        "mock": True,
    }
...
async def get_kubernetes_status(
    node_id: str, mock: bool = False, user: dict = Depends(get_current_user)
):
    if mock:
    ...
    results, err = await session_manager.run(
        node_id, node, [cmd_pods, cmd_nodes, cmd_deps], timeout=10.0
    )
    ...
    if not err and results and len(results) == 3:
        try:
            ...
            if (
                "command not found" not in results[0]["output"].lower()
                and "{" in results[0]["output"]
            ):
                ...
                for item in pods_raw.get("items", []):
                    ...
                    if status_phase == "Running" and any(
                        not cs.get("ready", True)
                        for cs in item.get("status", {}).get("containerStatuses", [])
                    ):
                ...
                for item in nodes_raw.get("items", []):
                    ...
                    status = (
                        "Ready" if ready_cond.get("status") == "True" else "NotReady"
                    )
                    ...
                    parsed_nodes.append(
                        {
                            ...
                            "memory_capacity": item.get("status", {})
                            .get("capacity", {})
                            .get("memory", "0Ki"),
                            "kubelet_version": item.get("status", {})
                            .get("nodeInfo", {})
                            .get("kubeletVersion", ""),
                        }
                    )
    ...
    if is_real and len(parsed_nodes) > 0:
        return {
            ...
            "cluster_health": (
                "Degraded"
                if any(p["status"] not in ("Running", "Succeeded") for p in parsed_pods)
                else "Healthy"
            ),
            "total_cpu": sum(n["cpu_capacity"] for n in parsed_nodes),
            "total_memory_gb": len(parsed_nodes)
            * 16,  # approximation since memory parsing from Ki to Gi is complex
            "mock": False,
        }
    else:
        raise HTTPException(
            status_code=404, detail="Kubernetes cluster not detected or kubectl failed."
        )
...
async def install_kubernetes(
    node_id: str, req: InstallRequest = None, user: dict = Depends(get_current_user)
):
    nodes_data = await load_nodes()
    if node_id not in nodes_data:
        raise HTTPException(status_code=404, detail="Node not found")
    ...
    sudo_prefix = "sudo"
    sudo_pass = (
        req.sudo_password if req and req.sudo_password else node.get("sudo_password")
    )
    if sudo_pass:
    ...
    results, err = await session_manager.run(
        node_id, node, [install_cmd], timeout=120.0
    )

    if err or not results:
        # Fetch the logs to see what went wrong
        log_res, _ = await session_manager.run(
            node_id, node, ["cat /tmp/k3s_install.log"], timeout=10.0
        )
        logs = (
            log_res[0]["output"]
            if log_res and isinstance(log_res[0], dict)
            else (log_res[0] if log_res else str(err))
        )
    ...
    await session_manager.run(
        node_id,
        node,
        [f"{sudo_prefix} rm -f /tmp/k3s_install.log /tmp/k3s_install.sh"],
        timeout=5.0,
    )
    ...
    await session_manager.run(
        node_id,
        node,
        [f"{sudo_prefix} chmod 644 /etc/rancher/k3s/k3s.yaml"],
        timeout=5.0,
    )
...
async def get_install_logs(node_id: str, user: dict = Depends(get_current_user)):
    ...
    results, err = await session_manager.run(
        node_id,
        node,
        ["cat /tmp/k3s_install.log 2>/dev/null || echo 'Waiting for logs...'"],
        timeout=10.0,
    )
    ...
    out = (
        results[0]["output"]
        if results and isinstance(results[0], dict)
        else (results[0] if results else "Error fetching logs")
    )
    return {"logs": out}
...
async def deploy_target(
    node_id: str, req: DeployRequest, user: dict = Depends(get_current_user)
):
    nodes_data = await load_nodes()
    if node_id not in nodes_data:
        raise HTTPException(status_code=404, detail="Node not found")
...
async def delete_pod(
    node_id: str, namespace: str, pod_name: str, user: dict = Depends(get_current_user)
):
    nodes_data = await load_nodes()
    if node_id not in nodes_data:
        raise HTTPException(status_code=404, detail="Node not found")
...
async def migrate_container(
    node_id: str, req: MigrateRequest, user: dict = Depends(get_current_user)
):
    nodes_data = await load_nodes()
    if node_id not in nodes_data:
        raise HTTPException(status_code=404, detail="Node not found")
    ...
    if err or not res or not res[0]["output"]:
        raise HTTPException(
            status_code=400, detail=f"Failed to inspect container: {err}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to parse inspect output: {e}"
        )
...
"""
    ...
    deploy_res, deploy_err = await session_manager.run(
        node_id, node, [cmd], timeout=30.0
    )
    ...
    out = (
        deploy_res[0]["output"]
        if deploy_res and isinstance(deploy_res[0], dict)
        else (deploy_res[0] if deploy_res else "")
    )
    return {
        "status": "success",
        "message": f"Migrated {name} to Kubernetes",
        "output": out,
    }
...
async def scale_deployment(
    ...
    if err or not results:
        raise HTTPException(
            status_code=500, detail=f"Failed to scale deployment: {err}"
        )
    ...
    return {
        "status": "success",
        "message": f"Scaled {name} to {req.replicas} replicas",
        "output": out,
    }
...
async def get_pod_logs(
    node_id: str, namespace: str, name: str, user: dict = Depends(get_current_user)
):
    nodes_data = await load_nodes()
    if node_id not in nodes_data:
        raise HTTPException(status_code=404, detail="Node not found")
