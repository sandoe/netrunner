...
async def get_node_usb_devices(node_id: str, user=Depends(get_current_user)):
    ...
    if not devices and session_manager.is_connected(node_id):
        # Try Linux/RPi first
        results, err = await session_manager.run(node_id, nodes[node_id], ["ls /dev/ttyUSB* /dev/ttyACM* /dev/ttyAMA* 2>/dev/null || true"])
                ...
                for line in output_str.split("\n"):
                    for p in line.split():
                        if p.strip().startswith("/dev/"):
                            devices.append({"device": p.strip(), "name": "Serial Device (via SSH)"})
                            
        # If still empty, try Windows PowerShell
        if not devices:
            win_results, win_err = await session_manager.run(node_id, nodes[node_id], ['powershell -Command "[System.IO.Ports.SerialPort]::GetPortNames()"'])
...
async def write_node_usb_serial(node_id: str, payload: SerialWritePayload, user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            resp = await client.post(agent_url, json={"port": payload.port, "data": payload.data, "baud": payload.baud})
            if resp.status_code != 200:
                raise HTTPException(status_code=500, detail=f"Agent responded with {resp.status_code}: {resp.text}")
        return {"status": "ok"}
        ...
        if session_manager.is_connected(node_id):
            try:
                ...
                raise HTTPException(status_code=503, detail=f"Agent unreachable and SSH fallback failed: {ssh_err}")
                
        raise HTTPException(status_code=503, detail=f"Failed to reach agent at {host} and SSH is disconnected: {e}")

class UsbNukePayload(BaseModel):
    port: str

@router.post("/{node_id}/nuke")
async def nuke_node_usb_serial(node_id: str, payload: UsbNukePayload, user=Depends(get_current_user)):
    nodes = await load_nodes_db()
    if node_id not in nodes:
        raise HTTPException(status_code=404, detail="Node not found")
        
    if not session_manager.is_connected(node_id):
        raise HTTPException(status_code=503, detail="SSH is disconnected. Connect to the node first.")
...
async def pause_proxy(node_id: str, payload: PortPayload, user=Depends(get_current_user)):
...
async def resume_proxy(node_id: str, payload: PortPayload, user=Depends(get_current_user)):
...
async def serial_monitor(ws: WebSocket, node_id: str, port: str = "", baud: int = 115200, reset: str = "false"):
    await ws.accept()
    try:
        import paramiko
    except ImportError:
    ...
    if not session_manager.is_connected(node_id):
        ...
        if not success:
            await ws.send_json({"type": "error", "data": f"SSH connect failed: {err}\\r\\n"})
            await ws.close()
            return
