...
async def get_stats(db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    stats = {
        "postgres": {"status": "offline", "tables": 0, "size": "Unknown"},
    ...
    stats["storage"] = {
        "postgres": {"used_mb": storage["postgres"]["size_mb"], "limit_mb": float(sys_settings.get("max_postgres_mb", 5000))},
        "influxdb": {"used_mb": storage["influxdb"]["size_mb"], "limit_mb": float(sys_settings.get("max_influxdb_mb", 10000))},
        "logs": {"used_mb": storage["logs"]["size_mb"], "limit_mb": float(sys_settings.get("max_logs_mb", 1000))}
    }

    return stats

@router.post("/query")
async def execute_query(payload: QueryPayload, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
...
async def run_exfiltration(payload: ExfiltratePayload, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    try:
        ...
        if not rows:
            return {"status": "ok", "message": "Source table is empty", "rows_migrated": 0}
            
        # Create target table dynamically if it doesn't exist
        columns = rows[0].keys()
        col_defs = ", ".join([f"{c} TEXT" for c in columns]) # Treat all as text for safety during exfiltration
        ...
        for row in rows:
            ...
            insert_q = text(f"INSERT INTO exfiltrated_{payload.target_table} ({cols}) VALUES ({placeholders})")
            await db.execute(insert_q, row)
            
        await db.commit()
        return {"status": "ok", "message": f"Successfully exfiltrated {len(rows)} rows.", "rows_migrated": len(rows)}
