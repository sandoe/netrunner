...
_bluetooth_broadcast_task: Optional[asyncio.Task] = None
...
async def bluetooth_ws(websocket: WebSocket):
    ...
    try:
        await websocket.send_text(
            json.dumps({"type": "init", "devices": list(discovered_devices.values())})
        )
        while True:
...
@router.get("/devices")
async def get_bluetooth_devices(user: dict = Depends(get_current_user)):
...
@router.post("/jam/{mac}")
async def engage_jammer(
    mac: str,
    node_id: str = None,
    user: dict = Depends(require_non_student),
):
    mac = mac.upper()
    if mac in jam_tasks:
...
async def enumerate_device(
    ...
    try:
        ...
        proc_scan = await asyncio.create_subprocess_exec(
                    ...
                    if "read" in char.properties:
                        try:
                            ...
                            try:
                                ...
                                srv_data["characteristics"][
                                    char.description or char.uuid
                                ] = decoded
                            except Exception:
                                srv_data["characteristics"][
                                    char.description or char.uuid
        ...
        if b"Paired: yes" not in stdout_info:
            ...
            if "InProgress" in out:
                await asyncio.create_subprocess_exec(
                    "bluetoothctl", "cancel-pairing", mac
                )
                return {
                    "status": "failed",
                    "message": "Pairing is already in progress. Please ensure the device is in pairing mode and try again.",
                }

            if not pairing_successful:
                return {
                    "status": "failed",
                    "message": out.strip()[-200:] or "Pairing failed (no output)",
                }
