...
def nmap_recon(target):
    ...
    try:
        res = subprocess.run(
            ["nmap", "-p", "443,902", "--script", "vmware-version,vmauthd-brute", target],
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
        )
        if "vmware-version" in res.stdout or "vmauthd" in res.stdout:
            log_result(target, "nmap_result", res.stdout, True)
        else:
            log_result(target, "nmap_result", "No VMware specific info found by Nmap.", False)
...
def unauth_api_check(target):
    ...
    if found:
        log_result(target, "api_endpoints", f"Discovered API endpoints: {', '.join(found)}", True)
    else:
        log_result(target, "api_endpoints", "No unauthenticated API endpoints found.", False)
...
def main():
    ...
    if not check_port(args.target, 443) and not check_port(args.target, 902):
        log_result(args.target, "error", "Ports 443 and 902 are closed. Not a VMware target.", False)
        return
