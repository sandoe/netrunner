...
async def run_test():
    ...
    for nid, node in nodes.items():
        if node.get("host") == "192.168.1.17":
            ...
            await sm.upload_file(nid, node, "/app/backend/scripts/mpremote_api.py", "/tmp/nr_scripts/mpremote_api.py")
            await sm.upload_file(nid, node, "/app/backend/scripts/nr_mcu_cli.py", "/tmp/nr_scripts/nr_mcu_cli.py")
            
            cmds = [
                ("LS", "python3 /tmp/nr_scripts/nr_mcu_cli.py /dev/ttyUSB0 ls /"),
                ("CREATE_TEST_FILE", "echo 'Hello from Antigravity SSH test!' > /tmp/test_file.txt"),
                ("WRITE", "python3 /tmp/nr_scripts/nr_mcu_cli.py /dev/ttyUSB0 write /hello.txt /tmp/test_file.txt"),
                ("READ", "python3 /tmp/nr_scripts/nr_mcu_cli.py /dev/ttyUSB0 read /hello.txt"),
                ("RM", "python3 /tmp/nr_scripts/nr_mcu_cli.py /dev/ttyUSB0 rm /hello.txt"),
            ]
