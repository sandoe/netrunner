...
def main():
    ...
    if not check_port(args.target, 2375):
        log_result(
            args.target,
            "error",
            "Port 2375 is closed. Docker Daemon is not exposed unauthenticated.",
            False,
        )
        return
    ...
    log_result(
        args.target,
        "recon",
        "Unprotected Docker Daemon found on port 2375. Fetching data...",
        True,
    )
    ...
    if containers:
        container_list = [
            f"{c.get('Names', [''])[0].strip('/')} ({c.get('Image')}) - {c.get('State')}"
            for c in containers
        ]
        log_result(
            args.target,
