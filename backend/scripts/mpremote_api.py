...
class MPRemoteError(Exception):
    pass


class NotConnectedError(MPRemoteError):
    pass
...
class StatResult:
    ...
    def is_dir(self) -> bool:
        return bool(stat_module.S_ISDIR(self.st_mode))

    @property
    def is_file(self) -> bool:
        return bool(stat_module.S_ISREG(self.st_mode))
...
class MPRemote:
    ...
    def connect(self, port: Optional[str] = None) -> None:
        with self._lock:
            if self._transport is not None:
                raise MPRemoteError("Already connected")
            if port is not None:
                self._port = port
            if self._port is None:
                candidates = self.list_ports()
                if not candidates:
                    raise MPRemoteError("No serial ports found")
    ...
    def enter_raw_repl(self, *, soft_reset: bool = False) -> None:
        with self._lock:
            ...
            try:
                ...
                raise _to_mpremote_error(exc, "enter_raw_repl") from None
            self._in_raw = True
    ...
    def exit_raw_repl(self) -> None:
        with self._lock:
            ...
            try:
                ...
                raise _to_mpremote_error(exc, "exit_raw_repl") from None
            self._in_raw = False
    ...
    def exec(self, code: str, *, output=None, timeout: Optional[float] = 10.0) -> str:
        with self._lock:
            self._ensure_raw()
            consumer = (
                (lambda b: output(b.decode("utf-8", "replace")))
                if output is not None
                else None
            )
            try:
                ret, err = self._transport.exec_raw(
                    code, timeout=timeout, data_consumer=consumer
                )
            except Exception as exc:
                raise _to_mpremote_error(exc, "exec") from None
            if err:
    ...
    def eval(self, expression: str) -> object:
        with self._lock:
            self._ensure_raw()
            try:
                return self._transport.eval(expression, parse=True)
            except Exception as exc:
                raise _to_mpremote_error(exc, "eval") from None
    ...
    def ls(self, path: str = "/") -> List[DirEntry]:
        ...
        with self._lock:
            ...
            try:
                ...
                raise _to_mpremote_error(exc, f"ls({path})") from None
        out = []
        for e in entries:
    ...
    def stat(self, path: str) -> StatResult:
        ...
        with self._lock:
            self._ensure_raw()
            try:
                raw = self._transport.fs_stat(path)
            except Exception as exc:
                raise _to_mpremote_error(exc, f"stat({path})") from None
    ...
    def exists(self, path: str) -> bool:
        ...
        with self._lock:
            self._ensure_raw()
            try:
                return bool(self._transport.fs_exists(path))
            except Exception as exc:
                raise _to_mpremote_error(exc, f"exists({path})") from None
    ...
    def isdir(self, path: str) -> bool:
        ...
        with self._lock:
            self._ensure_raw()
            try:
                return bool(self._transport.fs_isdir(path))
            except Exception as exc:
                raise _to_mpremote_error(exc, f"isdir({path})") from None
    ...
    def tree(self, path: str = "/") -> dict:
        path = self._norm(path)
        with self._lock:
            return self._tree_node(path, name=PurePosixPath(path).name or "/")
    ...
    def read_bytes(self, path: str, *, chunk_size: int = 256, progress=None) -> bytes:
        ...
        with self._lock:
            ...
            try:
                data = self._transport.fs_readfile(
                    path, chunk_size=chunk_size, progress_callback=cb
                )
            except Exception as exc:
                raise _to_mpremote_error(exc, f"read_bytes({path})") from None
        if progress is not None:
            progress(len(data), len(data), path)
        return bytes(data)
    ...
    def read_text(
        ...
        *,
        ...
        return self.read_bytes(path, chunk_size=chunk_size, progress=progress).decode(
            encoding
        )

    def write_bytes(
        self, path: str, data: bytes, *, chunk_size: int = 256, progress=None
    ) -> None:
        path = self._norm(path)
        if isinstance(data, str):
            raise TypeError("write_bytes expects bytes; use write_text for str.")
        with self._lock:
            self._ensure_raw()
            cb = self._wrap_progress(progress, path)
            try:
                self._transport.fs_writefile(
                    path, bytes(data), chunk_size=chunk_size, progress_callback=cb
                )
            except Exception as exc:
                raise _to_mpremote_error(exc, f"write_bytes({path})") from None
        if progress is not None:
            progress(len(data), len(data), path)
    ...
    def write_text(
        ...
        *,
        ...
        self.write_bytes(
            path, text.encode(encoding), chunk_size=chunk_size, progress=progress
        )
    ...
    def put(
        ...
        *,
        ...
        if not local_path.exists():
            raise MPRemoteError(f"Local path does not exist: {local_path}")
        if remote is None:
            remote = "/" + local_path.name
        elif remote.endswith("/"):
            remote = self._join(remote, local_path.name)
        ...
        with self._lock:
            ...
            if not recursive:
                raise MPRemoteError(
                    f"{local_path} is a directory; pass recursive=True to copy it."
                )
    ...
    def get(
        ...
        *,
        ...
        remote = self._norm(remote)
        local_path = (
            Path(local) if local is not None else Path(PurePosixPath(remote).name)
        )
        with self._lock:
            self._ensure_raw()
            if not self.isdir(remote):
                if local_path.is_dir():
                    local_path = local_path / PurePosixPath(remote).name
            ...
            if not recursive:
                raise MPRemoteError(
                    f"{remote} is a directory; pass recursive=True to copy it."
                )
            ...
            for entry in self.walk(remote):
                ...
                if entry.is_dir:
                    ...
                    dest.write_bytes(
                        self.read_bytes(
                            entry.path, chunk_size=chunk_size, progress=progress
                        )
                    )
            return local_path
    ...
    def mkdir(self, path: str, *, exist_ok: bool = False) -> None:
        ...
        with self._lock:
            ...
            if exist_ok and self.exists(path):
                return
            try:
                self._transport.fs_mkdir(path)
            except Exception as exc:
                raise _to_mpremote_error(exc, f"mkdir({path})") from None
            ...
            for part in parts:
                ...
                if self.exists(cur):
                    if not self.isdir(cur):
                        raise MPRemoteError(f"Path component is not a directory: {cur}")
                    continue
                try:
                    self._transport.fs_mkdir(cur)
                except Exception as exc:
                    raise _to_mpremote_error(exc, f"makedirs({cur})") from None
            if not parts and not exist_ok:
                raise MPRemoteError("Refusing to create root directory.")
    ...
    def rmdir(self, path: str) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                raise _to_mpremote_error(exc, f"rmdir({path})") from None
    ...
    def remove(self, path: str) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                raise _to_mpremote_error(exc, f"remove({path})") from None
    ...
    def touch(self, path: str) -> None:
        ...
        with self._lock:
            self._ensure_raw()
            try:
                self._transport.fs_touchfile(path)
            except Exception as exc:
                raise _to_mpremote_error(exc, f"touch({path})") from None

    def rename(self, src: str, dst: str) -> None:
        src, dst = self._norm(src), self._norm(dst)
        with self._lock:
            self.exec(f"import os; os.rename({src!r}, {dst!r})")
    ...
    def df(self, path: str = "/") -> DfInfo:
        ...
        try:
            total, used, free, frsize = ast.literal_eval(out)
        except Exception as exc:
            raise MPRemoteError(f"df({path}): could not parse result {out!r}: {exc}")
    ...
    def hashfile(
        ...
        with self._lock:
            self._ensure_raw()
            try:
                result = self._transport.fs_hashfile(path, algo, chunk_size=chunk_size)
            except Exception as exc:
                raise _to_mpremote_error(exc, f"hashfile({path})") from None
    ...
    def soft_reset(self) -> None:
        with self._lock:
            ...
            try:
                ...
                raise _to_mpremote_error(exc, "soft_reset") from None
            self._in_raw = True
    ...
    def hard_reset(self, *, disconnect: bool = True) -> None:
        with self._lock:
            self._ensure_raw()
            try:
                self._transport.exec_raw_no_follow("import machine; machine.reset()")
            except Exception as exc:
                raise _to_mpremote_error(exc, "hard_reset") from None
            time.sleep(0.1)
            if disconnect:
                self.disconnect()
    ...
    def _walk_into(self, path: str, acc: List[DirEntry]) -> None:
        for entry in self.ls(path):
            acc.append(entry)
            if entry.is_dir:
                self._walk_into(entry.path, acc)
    ...
    def _tree_node(self, path: str, name: str) -> dict:
        if self.isdir(path):
            ...
            for entry in self.ls(path):
                if entry.is_dir:
                    node["children"].append(self._tree_node(entry.path, entry.name))
                else:
