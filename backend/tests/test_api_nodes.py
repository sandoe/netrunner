...
INTERNAL_HEADERS = {"X-Netrunner-Internal-Token": "test-internal-token"}
...
@pytest.mark.asyncio
async def test_internal_node_creds_for_telnet(
    ...
    # The authenticated mux fetches credentials here — must be 200, not 500.
    r = await anon_client.get(
        f"/api/internal/node/{node_id}", headers=INTERNAL_HEADERS
    )
    ...
    assert (
        await anon_client.get("/api/internal/node/nope", headers=INTERNAL_HEADERS)
    ).status_code == 404


@pytest.mark.asyncio
async def test_edit_without_password_keeps_it(
    client: AsyncClient, anon_client: AsyncClient
):
    ...
    creds = (
        await anon_client.get(
            f"/api/internal/node/{nid}", headers=INTERNAL_HEADERS
        )
    ).json()
